...
#define APP_STREAMER_MSG_QUEUE_FILE     "app_queue_file"
#define STREAMER_TASK_NAME_FILE         "Streamer_file"
#define STREAMER_MESSAGE_TASK_NAME_FILE "StreamerMessage_file"
...
OsaThread msg_thread_file;

static int streamer_initialized = 0;
/*!
...
static void STREAMER_MessageTask_file(void *arg)
{
    OsaMq mq;
    STREAMER_MSG_T msg;
    streamer_handle_t *handle;
    ...
    ret = osa_mq_open(&mq, APP_STREAMER_MSG_QUEUE_FILE, STREAMER_MSG_SIZE, true);
    if (ERRCODE_NO_ERROR != ret)
    {
        PRINTF("osa_mq_open failed: %d\r\n", ret);
        return;
    ...
    osa_mq_close(&mq);
    osa_mq_destroy(APP_STREAMER_MSG_QUEUE_FILE);

    osa_thread_destroy(msg_thread_file);
}
...
status_t STREAMER_file_Create(streamer_handle_t *handle, char *filename, int eap_par)
{
    ...
    /* Create message process thread */
    osa_thread_attr_init(&thread_attr);
    osa_thread_attr_set_name(&thread_attr, STREAMER_MESSAGE_TASK_NAME_FILE);
    osa_thread_attr_set_stack_size(&thread_attr, STREAMER_MESSAGE_TASK_STACK_SIZE);
    ret = osa_thread_create(&msg_thread_file, &thread_attr, STREAMER_MessageTask_file, (void *)handle);
    osa_thread_attr_destroy(&thread_attr);
    if (ERRCODE_NO_ERROR != ret)
    {
        return kStatus_Fail;
    ...
    /* Create streamer */
    strcpy(params.out_mq_name, APP_STREAMER_MSG_QUEUE_FILE);
    params.stack_size = STREAMER_TASK_STACK_SIZE;
#ifdef EAP_PROC
...
#endif
    params.task_name    = STREAMER_TASK_NAME_FILE;
    params.in_dev_name  = "";
    params.out_dev_name = "";
...
void STREAMER_Init(void)
{
    ...
    streamer_pcm_init();
    streamer_initialized = 1;
}

void STREAMER_Init_file(void)
{
	if (streamer_initialized==1)
	{
		add_module_name(LOGMDL_GENERAL, "STREAMER_FILE");
		return;
	}
    /* Initialize OSA*/
    osa_init();
