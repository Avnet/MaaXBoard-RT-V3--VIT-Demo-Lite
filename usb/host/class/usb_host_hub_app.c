...
static void USB_HostHubGetInterruptStatus(usb_host_hub_instance_t *hubInstance)
{
    ...
    {
#ifdef HOST_ECHO
        PRINTF("error in hub interrupt recv\r\n");
#endif
    }
    else
    {
    ...
    switch (appStatus)
    {
        ...
        case kHubRunWaitSetInterface:
                                         ...
                                         hubInstance) == kStatus_USB_Success)
            {
                ...
                processSuccess           = 1U;
#ifdef HOST_ECHO
                PRINTF("hub get descriptor 7\r\n");
#endif
            }
            else
            {
#ifdef HOST_ECHO
                PRINTF("hub get descriptor 7 error\r\n");
#endif
                break;
            }
            break;
        ...
        case kHubRunGetDescriptor7:
            ...
            */
            ...
            {
#ifdef HOST_ECHO
                PRINTF("port number is bigger than USB_HOST_HUB_MAX_PORT\r\n");
#endif
                return;
            }
                                         ...
                                         USB_HostHubControlCallback, hubInstance) == kStatus_USB_Success)
            {
                ...
                processSuccess           = 1U;
#ifdef HOST_ECHO
                PRINTF("hub get descriptor\r\n");
#endif
            }
            else
            {
#ifdef HOST_ECHO
                PRINTF("hub get descriptor error\r\n");
#endif
                break;
            }
            break;
        ...
        case kHubRunSetPortPower:
            ...
            if (NULL == hubInstance->portList)
            {
                ...
                {
#ifdef HOST_ECHO
                    PRINTF("port list allocate fail\r\n");
#endif
            ...
            if (hubInstance->portIndex < hubInstance->portCount)
            {
                                              ...
                                              USB_HostHubControlCallback, hubInstance) == kStatus_USB_Success)
                {
                    ...
                    processSuccess           = 1U;
#ifdef HOST_ECHO
                    PRINTF("set port feature PORT_POWER\r\n");
#endif
                }
                else
                {
#ifdef HOST_ECHO
                    PRINTF("set port feature PORT_POWER fail\r\n");
#endif
...
#ifdef HOST_ECHO
                PRINTF("hub: C_PORT_RESET when detached\r\n");
#endif
            }
...
#ifdef HOST_ECHO
                PRINTF("hub: C_PORT_ENABLE when detached\r\n");
#endif
            }
...
#ifdef HOST_ECHO
                PRINTF("hub: C_PORT_OVER_CURRENT when detached\r\n");
#endif
            }
            else
            {
...
#ifdef HOST_ECHO
                PRINTF("PORT_CONNECTION in attach for detach\r\n");
#endif
            }
            else
            {
        ...
        if (0U != ((0x01U << (portIndex & 0x07U)) & (hubInstance->hubBitmapBuffer[portIndex >> 3U])))
        {
            if (portIndex == 0U) /* hub status change */
            {
                    ...
                    ((hubGlobal->hubProcess == hubInstance) && (hubInstance->portProcess == 0U)))
                {
                    ...
                    {
#ifdef HOST_ECHO
                        PRINTF("error in usb_class_hub_get_status\r\n");
#endif
                        hubInstance->hubStatus = (uint8_t)kHubRunIdle;
                    }
                    else
...
static void USB_HostHubRemoteWakeupCallback(void *param, usb_host_transfer_t *transfer, usb_status_t status)
{
    ...
    if (kStatus_USB_Success != status)
    {
        (void)PRINTF("Transfer failed to set remote wakeup request to HUB.\r\n");
    }
    ...
    if (kStatus_USB_Success == status)
    {
        ...
        while (NULL != hubInstance)
        {
            ...
            {
                (void)PRINTF("Set HUB remote wakeup feature: level %d, address %d.\r\n",
                ...
                if (kStatus_USB_Success != status)
                {
                    (void)PRINTF("Send set remote wakeup request to HUB failed.\r\n");
                }
                break;
            }
            hubInstance = hubInstance->next;
    ...
    if (NULL == hubInstance)
    {
        ...
        if (NULL == deviceInstance)
        {
            ...
            if (kStatus_USB_Success != status)
            {
                (void)PRINTF("Suspend USB BUS failed.\r\n");
            }
        }
        else
        {
            ...
            if (NULL != hubInstance4Device)
            {
                ...
                if (kStatus_USB_Success != status)
                {
                    (void)PRINTF("Send suspend request to HUB is failed.\r\n");
                }
            }
            else
            {
                (void)PRINTF("Invalid HUB instance of device.\r\n");
            }
        }
        if (kStatus_USB_Success != status)
        {
                                           ...
                                           void *callbackParam)
{
    ...
    {
#ifdef HOST_ECHO
        PRINTF("Error in sending hub set report!\r\n");
#endif
...
void USB_HostHubInterruptInCallback(void *param, uint8_t *data, uint32_t data_len, usb_status_t status)
{
    ...
    {
#ifdef HOST_ECHO
        PRINTF("hub interrupt in data callback error\r\n");
#endif
    ...
    switch (hostEventCode)
    {
            ...
            for (interfaceIndex = 0U; interfaceIndex < configuration->interfaceCount; ++interfaceIndex)
            {
                ...
                else
                {
                    ...
                    if (infoValue > 5U)
                    {
                        ...
                        (void)PRINTF("Host can support max 5 level hubs\r\n");
#endif
                        continue;
                    }
        ...
        case kUSB_HostEventDetach:
            ...
            if (hubInstance != NULL)
            {
                ...
                (void)PRINTF("hub detached:level=%u ", infoValue);
                (void)USB_HostHelperGetPeripheralInformation(deviceHandle, (uint32_t)kUSB_HostGetDeviceAddress,
                                                             &infoValue);
                (void)PRINTF("address=%u\r\n", infoValue);
